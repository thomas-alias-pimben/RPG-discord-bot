...
function  affPv(userId) 
{
  ...
  let retour="\n\n";
    ...
    retour += "PV : "+config[perso]["pv"][0].PV+" / "+Object.values(config[perso]["attribut"][0])[0]+"\n";
    retour += "PS : "+config[perso]["pv"][0].PS+" / "+Object.values(config[perso]["attribut"][0])[1]+"\n"
...
function  afficherPerso(userId)
{
  ...
  for(objectToInspect = config[perso];
    objectToInspect !== null;
    objectToInspect = Object.getPrototypeOf(objectToInspect) )
    {
      donnees = donnees.concat(Object.keys(objectToInspect));
...
function affSocial(userId) 
{
  ...
  for(objectToInspect = config[perso]["magie"];
    objectToInspect !== null;
    objectToInspect = Object.getPrototypeOf(objectToInspect) )
    {
...
function modifBonus(bonus) {
  ...
  fs.writeFileSync('./sourcePerso/autre.json',JSON.stringify(configautre,null,4));
}
function valeurBonus(bonus)
    ...
    config[perso]["attribut"].forEach(element => {
      ...
      clé.forEach(key => {
        if (key == att)
        {
          ...
          if (modif=="+1" || modif=="-1") 
          {
            config[perso]["attribut"][tmp][att] =  config[perso]["attribut"][tmp][att]+bonus;
          }
          else
          {
            if (!isNaN(bonus)) {
  ...
  function creerPNJ(perso)
  {
    if(configPNJ[perso.nom] == undefined)
    {
      ...
      fs.writeFileSync('./sourcePerso/PNJ.json',JSON.stringify(configPNJ,null,4))
    }
    else
    {
  ...
  function creerObjPerso(perso)
  {
    ...
    Object.keys(perso).forEach(element => {
      ...
      else if (element  =="nommagie1"||element=="nommagie2"||element == "nbmagie1"||element == "nbmagie2") 
      {
  ...
  function ecrireConfig()
  {
    ...
    for (const [key, value] of Object.entries(config)) {
      ...
      fs.writeFileSync('./perso/'+key+'.json',JSON.stringify(value,null,4));
    }
  }
